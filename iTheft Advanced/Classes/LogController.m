...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    return [[itheftLogger getLogArray] count];
}
...
- (void) reloadLogTable {
    [logArray release];
    logArray = [[itheftLogger getLogArray] retain];
	...
	if (actionSheet.tag == DELETE_LOG_BUTTON){
        ...
        if (buttonIndex == 1){
            if ([MFMailComposeViewController canSendMail]) {
                ...
                [mailViewController setSubject:@"itheft iOS Log"];
                [mailViewController setMessageBody:[itheftLogger logAsText] isHTML:NO];
                [mailViewController setToRecipients:[NSArray arrayWithObject:@"itheft-ios-devs@usefork.com"]];
            ...
            {
                itheftLogMessageAndFile(@"LogController",0,@"Device is unable to send email in its current state.");
                [self reloadLogTable];
            }
        }
}
