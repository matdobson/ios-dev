...
static itheftConfig *_instance = nil;

+(itheftConfig *)instance  {
	
	
	@synchronized([itheftConfig class]) {
		if(!_instance) {
			_instance = [[itheftConfig alloc] init];
...
{
	itheftConfig *newConfig = [[itheftConfig alloc] init];
...
- (void) updateMissingStatus {
	if (self.deviceKey != nil && ![self.deviceKey isEqualToString:@""]){
		itheftLogMessage(@"itheftConfig", 10, @"Updating missing status...");
